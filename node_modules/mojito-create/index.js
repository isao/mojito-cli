...
function run(args, opts, callback) {
    var port = opts.port || config.port,
        force = 'force' in opts ? opts.force : config.force,
        mungename = 'mungename' in opts ? opts.mungename : config.mungename,
        sources = [config.sources],
        subject = args[0];

    if (opts.sources) {
        sources.concat(opts.sources);
    }


}

function run_(params, options, callback) {
